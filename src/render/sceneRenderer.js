...
export class SceneRenderer {
  constructor(container,library,{quality='balanced',onContextLost=null}={}){
    ...
    this.ghost=null; this.markerGroup=new THREE.Group(); this.scene.add(this.markerGroup);
  ...
  async sync(entities,customGeometries=new Map()){
    ...
    for(const [id,obj] of [...this.customMeshes])if(!entities.has(id)){this.scene.remove(obj);this.customMeshes.delete(id);}
    this.invalidate();
  }
  ...
  setSelection(ids,entities,library=this.library){
    for(const h of this.selectionHelpers)this.scene.remove(h);this.selectionHelpers=[];
  ...
  fit(ids,entities){
    let box=new THREE.Box3();let found=false;for(const id of ids){const e=entities.get(id);if(!e)continue;let b;if(e.custom){const o=this.customMeshes.get(id);if(o)b=new THREE.Box3().setFromObject(o);}else{const d=this.library.get(e.partId);if(d)b=new THREE.Box3(new THREE.Vector3(...d.bbox[0]),new THREE.Vector3(...d.bbox[1])).applyMatrix4(mat4(e.matrix));}if(b){box.union(b);found=true;}}
    if(!found){for(const e of entities.values()){if(e.hidden)continue;const p=new THREE.Vector3().setFromMatrixPosition(mat4(e.matrix));box.expandByPoint(p);found=true;}}
    if(!found)return;const c=box.getCenter(new THREE.Vector3()),size=Math.max(20,box.getSize(new THREE.Vector3()).length());this.orbit.target.copy(c);const dir=this.camera.position.clone().sub(this.orbit.target).normalize();this.camera.position.copy(c.clone().add(dir.multiplyScalar(size*1.15)));this.camera.near=Math.max(.1,size/1000);this.camera.far=Math.max(5000,size*20);this.camera.updateProjectionMatrix();this.orbit.update();this.invalidate();
  }
  async setGhost(partId,matrix,snapped=false){
    const g=await this.library.geometry(partId); if(!this.ghost||this.ghost.userData.partId!==partId){if(this.ghost)this.scene.remove(this.ghost);this.ghost=new THREE.Mesh(g,new THREE.MeshStandardMaterial({color:snapped?0x5de397:0x62b0ff,transparent:true,opacity:.42,depthWrite:false}));this.ghost.userData.partId=partId;this.scene.add(this.ghost);}this.ghost.material.color.set(snapped?0x5de397:0x62b0ff);this.ghost.matrixAutoUpdate=false;this.ghost.matrix.copy(matrix);this.ghost.visible=true;this.invalidate();
  }
  clearGhost(){if(this.ghost){this.ghost.visible=false;this.invalidate();}}
  setMarkers(points){
    this.markerGroup.clear();const geo=new THREE.SphereGeometry(1.8,8,6);for(const p of points.slice(0,60)){const m=new THREE.Mesh(geo,new THREE.MeshBasicMaterial({color:p.color||0xf4d35e,transparent:true,opacity:.9}));m.position.fromArray(p.point);this.markerGroup.add(m);}this.invalidate();
  }
