...
export function isConnector(a){return a?.type==='pin'||a?.type==='shaft'||a?.type==='standoff';}
...
export function compatible(a,b){
  ...
  if(isConnector(a)&&isReceptacle(b))return true;
  if(isConnector(b)&&isReceptacle(a))return true;
  // Shaft-to-shaft is retained for explicit axle continuation points. Generic
  // connector-to-connector remains physically invalid.
