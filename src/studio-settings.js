...
function injectUI(){
  ...
  const btn=document.createElement('button');btn.id='settingsBtn';btn.className='settings-button';btn.textContent='Settings';btn.title='Controls, graphics and keybinds';
  ...
  const panel=document.createElement('section');panel.id='settingsPanel';panel.className='settings-panel';panel.setAttribute('aria-hidden','true');panel.innerHTML=`
    <div class="settings-card" role="dialog" aria-modal="true" aria-label="VEX CAD settings">
      <div class="settings-head"><div><b>Settings</b><small>Graphics, navigation and keybinds</small></div><button id="closeSettingsBtn" aria-label="Close settings">×</button></div>
      <div class="settings-scroll">
        <section class="settings-section"><div class="settings-section-title">Navigation</div>
          ...
          <div id="robloxControls" class="settings-subgroup">
            ...
            <div class="control-cheatsheet"><b>Roblox Studio preset</b><span>RMB + mouse: look</span><span>WASD: fly</span><span>Q / E: down / up</span><span>MMB + drag: pan</span><span>LMB + drag: box select</span><span>Wheel: move camera forward/back</span></div>
          </div>
        </section>
        <section class="settings-section"><div class="settings-section-title">Keybinds</div><div id="keybindGrid" class="keybind-grid"></div><small class="settings-note">Click a binding, then press the new key. Roblox mode uses the movement bindings while the viewport is active.</small></section>
  ...
  $('resetSettingsBtn').onclick=()=>{settings=normalizeSettings(structuredClone(DEFAULT_SETTINGS));saveSettings(settings);syncUI();applySettings();};
  $('controlPreset').onchange=e=>update({controlsPreset:e.target.value});$('materialStyle').onchange=e=>update({materialStyle:e.target.value});$('shadowMode').onchange=e=>update({shadowMode:e.target.value});$('showGrid').onchange=e=>update({showGrid:e.target.checked});$('showAxes').onchange=e=>update({showAxes:e.target.checked});$('invertY').onchange=e=>update({invertY:e.target.checked});
  for(const id of ['lookSensitivity','moveSpeed','panSensitivity','exposure'])$(id).oninput=e=>update({[id]:Number(e.target.value)});
  ...
  renderKeybinds();syncUI();
}
function renderKeybinds(){const grid=$('keybindGrid');if(!grid)return;grid.innerHTML=Object.keys(DEFAULT_SETTINGS.bindings).map(action=>`<div class="keybind-row"><span>${labels[action]||action}</span><button class="bind-button" data-bind="${action}">${prettyKey(settings.bindings[action])}</button></div>`).join('');for(const b of grid.querySelectorAll('[data-bind]'))b.onclick=()=>{recordingBinding=b.dataset.bind;b.textContent='Press a key…';b.classList.add('recording');};}
function syncUI(){if(!$('settingsPanel'))return;$('controlPreset').value=settings.controlsPreset;$('materialStyle').value=settings.materialStyle;$('shadowMode').value=settings.shadowMode;$('showGrid').checked=settings.showGrid;$('showAxes').checked=settings.showAxes;$('invertY').checked=settings.invertY;for(const id of ['lookSensitivity','moveSpeed','panSensitivity','exposure']){$(id).value=settings[id];$(`${id}Value`).textContent=id==='moveSpeed'?`${settings[id]} mm/s`:Number(settings[id]).toFixed(id==='panSensitivity'?1:2);}$('robloxControls').classList.toggle('disabled',settings.controlsPreset!=='roblox');$('settingsQuality').value=$('quality')?.value||'balanced';renderKeybinds();updateFooter();}
function update(patch){settings=saveSettings({...settings,...patch,bindings:{...settings.bindings,...(patch.bindings||{})}});syncUI();applySettings();}
function applySettings(){if(!renderer)return;renderer.applyVisualSettings(settings);renderer.setCameraSettings(settings);renderer.setNavigationPreset(settings.controlsPreset);updateFooter();}
...
function openSettings(){$('settingsPanel').classList.add('open');$('settingsPanel').setAttribute('aria-hidden','false');syncUI();}
function closeSettings(){recordingBinding=null;$('settingsPanel')?.classList.remove('open');$('settingsPanel')?.setAttribute('aria-hidden','true');renderKeybinds();}
...
function bindRobloxInput(){
  ...
  window.addEventListener('keydown',e=>{
    if(recordingBinding){e.preventDefault();e.stopImmediatePropagation();if(e.key==='Escape'){recordingBinding=null;renderKeybinds();return;}const key=eventKey(e);update({bindings:{[recordingBinding]:key}});recordingBinding=null;renderKeybinds();return;}
    if(settings.controlsPreset!=='roblox'||typing())return;
  ...
  window.addEventListener('blur',()=>{for(const a of [...movementActions,'fast'])renderer.setRobloxKey(a,false);renderer.navPointer=null;leftStart=null;robloxBox=false;});
}
