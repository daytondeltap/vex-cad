...
async function snapshot(){return page.evaluate(()=>({url:location.href,title:document.title,booted:window.__vexBooted,bootError:window.__vexBootError||'',status:document.querySelector('#status')?.textContent||'',meta:document.querySelector('#libraryMeta')?.textContent||'',perf:document.querySelector('#perf')?.textContent||'',canvasCount:document.querySelectorAll('#viewport canvas').length,partRows:document.querySelectorAll('.part-row').length,body:(document.body?.innerText||'').slice(0,700)}));}
try{
  ...
  console.log(JSON.stringify({ok:true,meta,graphics,status,errors,badResponses,failedRequests}));
} catch(err){
