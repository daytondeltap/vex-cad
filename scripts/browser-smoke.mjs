...
const setControl=async(selector,value,event='change')=>page.locator(selector).evaluate((el,args)=>{if(el.type==='checkbox')el.checked=!!args.value;else el.value=String(args.value);el.dispatchEvent(new Event(args.event,{bubbles:true}));},{value,event});
try{
  ...
  const panelState=await page.locator('#settingsPanel').evaluate(el=>{const r=el.getBoundingClientRect(),s=getComputedStyle(el);return {className:el.className,aria:el.getAttribute('aria-hidden'),display:s.display,visibility:s.visibility,opacity:s.opacity,width:r.width,height:r.height};});
  ...
  await page.locator('#doneSettingsBtn').evaluate(el=>el.click());
  ...
  await page.keyboard.up('w');await page.locator('#doneSettingsBtn').evaluate(el=>el.click());
  ...
  await page.locator('#previewTutorialBtn').evaluate(el=>el.click());await page.locator('#tutorialViewer.open').waitFor({state:'visible',timeout:7000});
  ...
  await page.locator('#exitTutorialViewer').evaluate(el=>el.click());
  ...
  await page.locator('#closeSimulationBtn').evaluate(el=>el.click());
